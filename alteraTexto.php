...
$textoPerfil=@mysql_query("SELECT * FROM users WHERE tituloPerfil LIKE '%$meuPerfil%';",$conexao);
$textoPerfil=@mysql_fetch_array($textoPerfil);
$textoAtividades=@mysql_query("SELECT * FROM users WHERE tituloPerfil LIKE '%$meuPerfil%';",$conexao);
$textoAtividades=@mysql_fetch_array($textoAtividades);
$textoHobbies=@mysql_query("SELECT * FROM users WHERE tituloPerfil LIKE '%$meuPerfil%';",$conexao);
$textoHobbies=@mysql_fetch_array($textoHobbies);
$textoEstado=@mysql_query("SELECT * FROM users WHERE tituloPerfil LIKE '%$meuPerfil%';",$conexao);
$textoEstado=@mysql_fetch_array($textoEstado);
$textoEstado=$textoEstado["estado"];
$textoCidade=@mysql_query("SELECT * FROM users WHERE tituloPerfil LIKE '%$meuPerfil%';",$conexao);
$textoCidade=@mysql_fetch_array($textoCidade);
$textoCidade=$textoCidade["cidade"];
$textoStatus=@mysql_query("SELECT * FROM users WHERE tituloPerfil LIKE '%$meuPerfil%';",$conexao);
$textoStatus=@mysql_fetch_array($textoStatus);
$textoStatus=$textoStatus["status"];
$textoEmail=@mysql_query("SELECT * FROM users WHERE tituloPerfil LIKE '%$meuPerfil%';",$conexao);
...
?>
<br><br>
<form method="post" action="incluirTexto.php?confirmacao=PERFIL ALTERADO">   <img src="BACKGROUNDS/Clov!System_2012_logo.png" width="120" 
height="60" align="right" id="body"/>
<?php
echo'Texto do Perfil:<BR><textarea name="textoPerfil" cols="40" rows="8" wrap="on" scrolling="yes" value="$textoPerfil">'.$textoPerfil["textoPerfil"].'</textarea>    <BR><BR>
Minhas Atividades:<BR><textarea  name="textoAtividades"  cols="40" rows="8" wrap="on" scrolling="yes" value="$textoAtvidades">'.$textoAtividades["atividades"].'</textarea> <BR><BR>
Meus Hobbies:<BR><textarea  name="textoHobbies"  cols="40" rows="8" wrap="on" scrolling="yes" value="$textoHobbies">'.$textoHobbies["hobby"].'</textarea>    <BR><BR>
Estado:<input type="text"  name="textoEstado" size="6" maxlength="2" id="Estado" value="'.$textoEstado.'"/>  <BR><BR>
Cidade:<input type="text"  name="textoCidade" size="30" id="Proper" value="'.$textoCidade.'"/>    <BR><BR>
senha:<input type="text" value="" name="textoSenha" size="30" />    <BR><BR>
Dia de anivers&aacute;rio:<select name="textoDia">
...
Status:<select name="textoStatus">';
...
<input type="text" value="'.$textoEmail.'" name="textoEmail" size="15" maxlength="40" id="Email"/>@
<select name="dominio">
...
<BR>
<BR>
<font color="green">Informe seu e-mail corretamente para alterar seu perfil. </font>
<BR>
<BR>
